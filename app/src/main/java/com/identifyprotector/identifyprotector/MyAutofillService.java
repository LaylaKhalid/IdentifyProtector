...
import com.identifyprotector.identifyprotector.data.ClientAutofillDataBuilder;
import com.identifyprotector.identifyprotector.data.ClientViewMetadata;
import com.identifyprotector.identifyprotector.data.ClientViewMetadataBuilder;
import com.identifyprotector.identifyprotector.data.DataCallback;
import com.identifyprotector.identifyprotector.data.adapter.DatasetAdapter;
import com.identifyprotector.identifyprotector.data.adapter.ResponseAdapter;
...
import com.identifyprotector.identifyprotector.data.source.local.DigitalAssetLinksRepository;
...
import com.identifyprotector.identifyprotector.model.DalCheck;
import com.identifyprotector.identifyprotector.model.DalInfo;
...
import static com.identifyprotector.identifyprotector.util.Util.DalCheckRequirement;
import static com.identifyprotector.identifyprotector.util.Util.bundleToString;
import static com.identifyprotector.identifyprotector.util.Util.dumpStructure;
import static com.identifyprotector.identifyprotector.util.Util.logVerboseEnabled;
import static com.identifyprotector.identifyprotector.util.Util.logd;
import static com.identifyprotector.identifyprotector.util.Util.loge;
import static com.identifyprotector.identifyprotector.util.Util.logv;
...
public class MyAutofillService extends AutofillService {
                ...
                new DataCallback<HashMap<String, FieldTypeWithHeuristics>>() {
                    ...
                    public void onLoaded(HashMap<String, FieldTypeWithHeuristics> fieldTypesByAutofillHint) {
                        ...
                        if (!mPackageVerificationRepository.putPackageSignatures(packageName)) {
                            callback.onFailure(getString(R2.string.invalid_package_signature));
                            return;
                        }
    ...
    private void fetchDataAndGenerateResponse(
        ...
        if (responseAuth) {
            ...
            RemoteViews remoteViews = RemoteViewsHelper.viewsWithAuth(getPackageName(),
                    getString(R2.string.autofill_sign_in_prompt));
                        ...
                        public void onLoaded(List<DatasetWithFilledAutofillFields> datasets) {
                            if ((datasets == null || datasets.isEmpty()) && manual) {
                                ...
                                RemoteViews remoteViews = RemoteViewsHelper.viewsWithNoAuth(
                                        getPackageName(),
                                        getString(R2.string.autofill_manual_prompt));
    ...
    private void checkWebDomainAndBuildAutofillData(String packageName, SaveCallback callback) {
        ...
        try {
            ...
            logw(e.getMessage());
            callback.onFailure(getString(R2.string.security_exception));
            return;
        }
                    ...
                    new DataCallback<DalCheck>() {
                        ...
                        public void onLoaded(DalCheck dalCheck) {
                            if (dalCheck.linked) {
                                ...
                                callback.onFailure(getString(R2.string.dal_exception));
                            }
                        }
